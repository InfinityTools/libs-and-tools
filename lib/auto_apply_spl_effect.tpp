 ...
 *
 * STR_VAR function_name  The WeiDU function to call. Supported functions:
 *                        ADD_SPELL_EFFECT, ADD_SPELL_CFEFFECT, ADD_ITEM_EFFECT, ADD_ITEM_EQEFFECT,
 *                        and (limited) ADD_CRE_EFFECT.
 * STR_VAR effect_codes   A string with one or more effect definitions. The general code format is
 *                        "param1=value1,param2=value2;...second effect...".
 *                        Each effect parameter consists of "key=value" pair and is separated by
 *                        comma (,) from other parameters of the same effect. A semicolon (;)
 *                        separates individual effect definitions.
 *                        Supported parameters:
 *                        op (Opcode), tgt (Target), tmg (Timing), pwr (Power), p1 (Parameter1),
 *                        p2 (Parameter2), rd (Resist/Dispel), dur (Duration), pro1 (Probability1),
 *                        pro2 (Probability2), res (Resource), dnum (Dice Number), dsize (Dice Size),
 *                        stype (Save Type), sbonus (Save Bonus), spec (Special), ip (Insert Point),
 *                        t (Type)
 *                        All parameters except "Opcode" are optional.
 */
DEFINE_PATCH_FUNCTION auto_apply_spl_effect
INT_VAR
...
STR_VAR
  ...
  def_resource      = ~~
BEGIN
  PATCH_MATCH ~%function_name%~ WITH
    ~ADD_SPELL_EFFECT~
    ~ADD_SPELL_CFEFFECT~
    ~ADD_ITEM_EFFECT~
    ~ADD_ITEM_EQEFFECT~
    ~ADD_CRE_EFFECT~ BEGIN
      SET valid_function = 1
    END
    DEFAULT
      SET valid_function = 0
  END

  PATCH_IF (valid_function) BEGIN
    ...
    END
  END ELSE BEGIN
    PATCH_WARN ~WARNING: Not a valid function: %function_name%~
  END
END